...
class Benchmark_UniBot:
    ...
    def run_benchmark(self,k):
        ...
        for q_id in self.manifest:
            curr_chain = ConvChain(self.model, self.retriever, self.document_store)
            res_dict = self.perform_test(curr_chain, q_id, res_dict, k)
    ...
    def perform_test(self, curr_chain, query_id: int, res_dict: dict,k:int, follow_up:bool = False):
        ...
        response = curr_chain.get_response(og_query,k)
        ...
        res_prec, res_recall = self.compute_set_precision_recall(curr_chain.retrieved_ids,
        ...
        if query_obj["type"] == "multi" and follow_up == False:
            curr_chain.retrieved_ids = []
            res_dict = self.perform_test(curr_chain,query_id, res_dict, k, True)
