...
SYSTEM_PROMPT = """You are a knowledgeable assistant helping users find accurate information from the website of the University of Luxembourg. 
# your role is to:
...
USER_PROMPT = """Answer the following instruction using only information from the provided documents if they are relevant.
...
class Gen_Model:
    ...
    def get_formatted_prompt(self, instruction, input=None, chat_history=None):
        ...
        message.append({"role": "user",
                        "content": USER_PROMPT.format(input=input, instruction=instruction)})
    ...
    def get_response(self, query_res, INSTRUCTION, chat_history, MAX_TOKENS=1024):
        ...
        chat_completion = self.model.chat.completions.create(
                            messages=self.get_formatted_prompt(instruction=INSTRUCTION,
                                                               input=query_res,
